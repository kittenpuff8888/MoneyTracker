  ...
  return (
    <>
      <Button
        ...
        className={compact ? "h-11 w-11 px-0 md:h-8 md:w-auto md:px-3" : "h-8 px-3"}
      ...
      {open ? (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/40 p-0 sm:items-center sm:p-4">
          <div
            ...
            className="w-full border border-border bg-white p-5 shadow-xl sm:max-w-md sm:rounded-lg"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 id="confirm-delete-title" className="text-lg font-semibold">Delete {itemName}?</h2>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">{warningText}</p>
              </div>
              <Button
                type="button"
                variant="ghost"
